...
public readonly partial struct UInt256
{
    ...
    public static void MultiplyMod(in UInt256 x, in UInt256 y, in UInt256 m, out UInt256 res)
    {
        ...
        if(y.IsOne)
        {
            Mod(in x, in m, out res);
            return;
        }
        if(x.IsOne)
        {
            Mod(in y, in m, out res);
            return;
        }
        ...
        Multiply256To512Bit(in x, in y, out var lo, out var hi);
    ...
    private static void ReduceSumAssumingLT2m(in UInt256 sum, ulong carry, in UInt256 m, out UInt256 res)
    {
        ...
        else if(mask == UInt64.MaxValue)
        {
            res = d;
        }
        else if(Vector256.IsHardwareAccelerated)
        {
            var dV = Unsafe.As<UInt256, Vector256<ulong>>(ref Unsafe.AsRef(in d));
            var sumV = Unsafe.As<UInt256, Vector256<ulong>>(ref Unsafe.AsRef(in sum));
            var maskV = Vector256.Create(mask);

            var resultV = Vector256.ConditionalSelect(dV, sumV, maskV);
        ...
        {
            var mask256 = Create(mask, mask, mask, mask);
    ...
    private static void Remainder257By256Bits(in UInt256 valueLo256, in UInt256 modulus256, out UInt256 remainder)
    {
        ...
        var normRem = Create(u0, u1, u2, u3);
        remainder = ShiftRightSmall(in normRem, normShiftBits);
    }

    [SkipLocalsInit]
    [MethodImpl(MethodImplOptions.NoInlining)]
    private static void ModFull(in UInt256 x, in UInt256 y, out UInt256 res) => DivideImpl(x, y, out _, out res);
    ...
    private static void Remainder512By256Bits(in UInt256 lo, in UInt256 hi, in UInt256 d, out UInt256 rem)
    {
        ...
        {
            dLen = 4;
            shift = BitOperations.LeadingZeroCount(d3);
        }
        else if(d2 != 0)
        {
            dLen = 3;
            shift = BitOperations.LeadingZeroCount(d2);
        }
        else if(d1 != 0)
        {
            dLen = 2;
            shift = BitOperations.LeadingZeroCount(d1);
        }
        else
        {
            dLen = 1;
            shift = BitOperations.LeadingZeroCount(d0);
        }
    ...
    private static void DivideBy192Bits(in UInt256 x, in UInt256 y, out UInt256 q, out UInt256 remainder)
    {
        ...
        // Remainder is u0..u2
        var remN = Create(u0n2, u1d, u2d, 0);
    ...
    private static void DivideBy256Bits(in UInt256 x, in UInt256 y, out UInt256 q, out UInt256 remainder)
    {
        ...
        var v = ShiftLeftSmall(in y, shift);
        // Normalise dividend u = x << shift into 5 limbs
        ulong u4d;
        if(shift == 0)
        {
        ...
        ulong borrow = SubMul4(in u, ref u4d, in v, qhat, out var rem);
    ...
    private static UInt256 ShiftLeftSmall(in UInt256 v, int sh)
    {
        ...
        else
        {
            ...
            {
                var prev = Avx512F.VL.AlignRight64(x, default, 3);
                var right = Avx2.ShiftRightLogical(prev, cR);
                y = Avx2.Or(left, right);
            }
            else
            {
                var right = Avx2.ShiftRightLogical(x, cR);
                var carry = Avx2.Permute4x64(right, 0x90);
                carry = Avx2.And(carry, Vector256.Create(0UL, UInt64.MaxValue, UInt64.MaxValue, UInt64.MaxValue));
    ...
    private static UInt256 ShiftRightSmall(in UInt256 v, int sh)
    {
        ...
        else
        {
            ...
            // right = x >> sh
            var cR = Vector128.CreateScalarUnsafe((ulong) sh);
            var right = Avx2.ShiftRightLogical(x, cR);

            // carry = (next limb) << (64 - sh)
            var cL = Vector128.CreateScalarUnsafe((ulong) (64 - sh));

            Vector256<ulong> y;
            if(Avx512F.VL.IsSupported)
            {
                // next = [a1, a2, a3, 0]
                var next = Avx512F.VL.AlignRight64(default, x, 1);
                var carry = Avx2.ShiftLeftLogical(next, cL);
            ...
            else
            {
                ...
                var carry = Avx2.Permute4x64(left, 0x39);
                carry = Avx2.And(carry, Vector256.Create(UInt64.MaxValue, UInt64.MaxValue, UInt64.MaxValue, 0UL));
                y = Avx2.Or(right, carry);
            }
