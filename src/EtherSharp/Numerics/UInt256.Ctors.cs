...
public readonly partial struct UInt256
{
    public UInt256(uint r0, uint r1, uint r2, uint r3, uint r4, uint r5, uint r6, uint r7)
    {
        if(Vector256.IsHardwareAccelerated)
        {
            ...
            Unsafe.As<ulong, Vector256<uint>>(ref u0) = Vector256.Create(r0, r1, r2, r3, r4, r5, r6, r7);
        }
        else
        {
            u0 = ((ulong) r1 << 32) | r0;
    ...
    public UInt256(in ReadOnlySpan<byte> bytes, bool isBigEndian = false)
    {
        if(bytes.Length == 32)
        {
            if(isBigEndian)
            {
                ...
                {
                    u3 = BinaryPrimitives.ReadUInt64BigEndian(bytes[..8]);
            ...
            else
            {
                ...
                {
                    u0 = BinaryPrimitives.ReadUInt64LittleEndian(bytes[..8]);
    ...
    public UInt256(in ReadOnlySpan<ulong> data, bool isBigEndian = false)
    {
        ...
        else
        {
            if(Vector256.IsHardwareAccelerated)
            {
                ...
                Unsafe.SkipInit(out u2);
                Unsafe.SkipInit(out u3);
                Unsafe.As<ulong, Vector256<ulong>>(ref u0) = Vector256.Create<ulong>(data);
            }
            else
            {
                u0 = data[0];
