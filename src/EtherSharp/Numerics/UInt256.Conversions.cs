...
{
    public void Convert(out BigInteger big) => big = (BigInteger) this;
    ...
    public void ToBigEndian(Span<byte> target)
    {
        if(target.Length == 32)
        {
            BinaryPrimitives.WriteUInt64BigEndian(target[..8], u3);
        ...
        {
            BinaryPrimitives.WriteUInt32BigEndian(target[..4], (uint) u2);
    ...
    public void ToLittleEndian(Span<byte> target)
    {
        if(target.Length == 32)
        {
            ...
            {
                BinaryPrimitives.WriteUInt64LittleEndian(target[..8], u0);
    ...
    public static UInt256 Parse(string value) => !TryParse(value, out var c) ? throw new FormatException() : c;

    public static UInt256 Parse(in ReadOnlySpan<char> value, NumberStyles numberStyles) => !TryParse(value, numberStyles, CultureInfo.InvariantCulture, out var c) ? throw new FormatException() : c;
    ...
    public static bool TryParse(ReadOnlySpan<char> value, out UInt256 result) => value.StartsWith("0x", StringComparison.OrdinalIgnoreCase)
        ? TryParse(value[2..], NumberStyles.HexNumber, CultureInfo.InvariantCulture, out result)
