...
public abstract class BaseTxScheduler : ITxScheduler, IInitializableService, IDisposable
{
    ...
    public BaseTxScheduler(IEtherSigner signer, ITxPublisher txPublisher)
    {
    ...
    public virtual ValueTask InitializeAsync(ulong chainId)
    {
        _chainId = chainId;
    ...
    protected string EncodeCallData<TTransaction, TTxParams>(TTxParams txParams, ITxInput txInput, uint nonce)
    ...
    protected void SignAndEncode(Span<byte> txTemplateBuffer, Span<byte> signatureBuffer, out int encodedSignatureLength)
    {
