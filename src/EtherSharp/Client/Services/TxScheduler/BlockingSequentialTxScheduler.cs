...
//    System.Threading.Tasks.TaskCompletionSource<EtherSharp.Tx.PendingTxHandler> CompletionSource
//);
...
//    async Task<PendingTxHandler> ITxScheduler.PublishTxAsync<TTransaction, TTxParams, TTxGasParams>(
//        ITxInput txInput, TTxParams? txParams, TTxGasParams? txGasParams)
//        where TTxParams : class
//        where TTxGasParams : class
//    {
//        var tcs = new TaskCompletionSource<PendingTxHandler>();
...
//        //ToDo: Consider updating this every while
//        txGasParams ??= await gasFeeProvider.EstimateGasParamsAsync(txInput.To, txInput.Value, txInput.Data, txParams, default);

//        string EncodeFunc(uint nonce) => handler.EncodeTxToBytes(txInput, txParams, txGasParams, txInput.Data, nonce);
//        return !_queue.Writer.TryWrite(( EncodeFunc, txInput, tcs))
...
//    {
//        var (encodeFunc, txInput, _) = entry;
//        uint nonce = Interlocked.Increment(ref _nonceCounter);

//        while(true)
//        {
//            string txBytes = encodeFunc(nonce);
...
//            {
//                TxConfirmationAction.ContinueWaiting waitAction => await _txConfirmer.WaitForTxConfirmationAsync(txHash, waitAction.Duration),
