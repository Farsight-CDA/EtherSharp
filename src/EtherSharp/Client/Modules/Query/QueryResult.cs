...
public record QueryResult<T>
{
    ...
    public static QueryResult<T> Parse(TxCallResult callResult, Func<ReadOnlyMemory<byte>, T> parser)
        => callResult switch
        {
            TxCallResult.Success s => new QueryResult<T>.Success(parser(s.Data)),
