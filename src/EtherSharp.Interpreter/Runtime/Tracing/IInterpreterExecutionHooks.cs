...
public interface IInterpreterExecutionHooks
{
    ...
    /// <summary>Runs before a contract call, creation, or precompile's entry checks.</summary>
    /// <remarks>Includes outer invocations, empty-code calls, and rejected entries.</remarks>
    ValueTask OnContractEnterAsync(IInterpreterFrame frame, IInterpreterStorage storage)
        => ValueTask.CompletedTask;

    /// <summary>Runs after a contract call, creation, or precompile finalizes.</summary>
    /// <remarks>Runs after rollback and, for creation, runtime-code validation. Shares the frame instance with entry and instruction callbacks.</remarks>
