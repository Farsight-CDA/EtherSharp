...
public sealed class AbiRoundTripTests
{
    ...
    {
        Bytes1[] input = [
            Bytes1.FromBytes([0x12]),
            Bytes1.FromBytes([0x34]),
            Bytes1.FromBytes([0xab]),
            Bytes1.FromBytes([0xcd])
        ];

        var encoder = new AbiEncoder().Bytes1Array(input);
        var decoder = new AbiDecoder(encoder.Build());

        var output = decoder.Bytes1Array();
    ...
    public void Should_Encode_And_Decode_Bytes32_Array()
    {
        // Arrange
        Bytes32[] input = [
            Bytes32.FromBytes([0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f]),
            Bytes32.FromBytes([0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff]),
            Bytes32.FromBytes([0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        ];
        ...
        {
            Assert.Equal(input[i].ToArray(), output[i].ToArray());
        }
    }
    ...
    public void Should_Encode_And_Decode_Bytes32_Array_With_Other_Types()
    {
        ...
        Bytes32[] bytes32Input = [
            Bytes32.FromBytes([0xde, 0xad, 0xbe, 0xef, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00]),
            Bytes32.FromBytes([0xca, 0xfe, 0xba, 0xbe, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
        ];
        ...
        {
            Assert.Equal(bytes32Input[i].ToArray(), outputBytes32[i].ToArray());
        }
        Assert.Equal(str, outputStr);
